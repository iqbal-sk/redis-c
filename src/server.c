    ...
    while (1)
    {
        ...
        for (int fd = 0; fd <= fdmax; fd++)
        {
            ...
            if (srv->repl_fd >= 0 && fd == srv->repl_fd)
            {
                ...
                if (mr <= 0)
                {
                    // master closed; stop tracking
                    FD_CLR(fd, &master_set); srv->repl_fd = -1; srv->repl_handshake_step = 0;
                }
                else
                {
                    ...
                    if (srv->repl_handshake_step == 1)
                    {
                        ...
                        if (l1 > 0 && (size_t)l1 < sizeof(rp1))
                            (void)send_all(fd, rp1, (size_t)l1);
                        srv->repl_handshake_step = 2;
                    }
                    else if (srv->repl_handshake_step == 2)
                    {
                        // Send REPLCONF capa psync2
                        const char *rp2 = "*3\r\n$8\r\nREPLCONF\r\n$4\r\ncapa\r\n$6\r\npsync2\r\n";
                        (void)send_all(fd, rp2, strlen(rp2));
                        srv->repl_handshake_step = 3;
                    }
                    else if (srv->repl_handshake_step == 3)
                    {
                        // Send PSYNC ? -1 (later stage, fine to send now if present)
                        const char *ps = "*3\r\n$5\r\nPSYNC\r\n$1\r\n?\r\n$2\r\n-1\r\n";
...
int server_connect_master(Server *srv, const char *host, int port)
{
    ...
    // Set handshake step: await PING response before sending REPLCONF
    srv->repl_handshake_step = 1;
    // Send PSYNC ? -1
    // const char *ps = "*3\r\n$5\r\nPSYNC\r\n$1\r\n?\r\n$2\r\n-1\r\n";
