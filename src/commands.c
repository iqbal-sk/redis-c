...
static const size_t kEmptyRDBLen = sizeof(kEmptyRDB);
...
static int handle_psync(int fd, Conn *c, DB *db, const Arg *args, size_t nargs)
{
    ...
    if (n <= 0 || (size_t)n >= sizeof(buf)) return -1;
    if (reply_simple(fd, buf) != 0) return -1;
    // Then send empty RDB as a bulk-like payload without trailing CRLF
    char h[64];
    int hl = snprintf(h, sizeof(h), "$%zu\r\n", kEmptyRDBLen);
